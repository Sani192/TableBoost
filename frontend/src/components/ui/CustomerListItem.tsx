          ...
          {showTags && (
            <div className="flex flex-wrap items-center gap-1 shrink-0">
              {isVip && hasFeatureAccess('smart_segments') && (
                <span className="inline-flex items-center gap-0.5 px-1 py-0.25 rounded text-[10px] font-bold bg-amber-100 text-amber-700 border border-amber-200">
              ...
              {hasFeatureAccess('intelligence') && (
                <>
                  {clvTier === 'high' && (
                    <span className="inline-flex items-center gap-0.5 px-1 py-0.25 rounded text-[10px] font-bold bg-purple-100 text-purple-700 border border-purple-200">
                      HIGH CLV
                    </span>
                  )}
                  {clvTier === 'medium' && (
                    <span className="inline-flex items-center gap-0.5 px-1 py-0.25 rounded text-[10px] font-bold bg-indigo-100 text-indigo-700 border border-indigo-200">
                      MEDIUM CLV
                    </span>
                  )}
                  {clvTier === 'low' && (
                    <span className="inline-flex items-center gap-0.5 px-1 py-0.25 rounded text-[10px] font-bold bg-stone-100 dark:bg-stone-800 text-stone-700 dark:text-stone-300 border border-stone-200 dark:border-stone-700">
                      LOW CLV
                    </span>
                  )}
                  {isNew && (
                    <span className="inline-flex items-center gap-0.5 px-1 py-0.25 rounded text-[10px] font-bold bg-blue-100 text-blue-700 border border-blue-200">
                      <Star className="w-2.5 h-2.5" /> NEW
                    </span>
                  )}
                  {isHealthy && (
                    <span className="inline-flex items-center gap-0.5 px-1 py-0.25 rounded text-[10px] font-bold bg-emerald-100 text-emerald-700 border border-emerald-200">
                  ...
                  {isDeclining && (
                    <span className="inline-flex items-center gap-0.5 px-1 py-0.25 rounded text-[10px] font-bold bg-yellow-100 text-yellow-700 border border-yellow-200">
                      DECLINING
                    </span>
                  )}
                  {isAtRisk && (
                    <span className="inline-flex items-center gap-0.5 px-1 py-0.25 rounded text-[10px] font-bold bg-orange-100 text-orange-700 border border-orange-200">
                      <UserCheck className="w-2.5 h-2.5" /> AT RISK
                    </span>
                  )}
                  {isLost && (
                    <span className="inline-flex items-center gap-0.5 px-1 py-0.25 rounded text-[10px] font-bold bg-red-100 text-red-700 border border-red-200">
